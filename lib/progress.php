 ...
 * @return array
 */
function itk_block_progress_monitorable_modules() {
    global $DB;
 ...
 * @return string
 */
function itk_progress_default_value(&$var, $def = null) {
 ...
 * @return array
 */
function itk_block_progress_modules_in_use($course) {
    global $DB;

    $dbmanager = $DB->get_manager(); // Used to check if tables exist.
    $modules = itk_block_progress_monitorable_modules();
    $modulesinuse = array();
 ...
 */
function itk_block_progress_event_information($config, $modules, $course, $userid = 0) {
    global $DB, $USER;
    $events = array();
    $numevents = 0;
    ...
    $sections = itk_block_progress_course_sections($course);
    ...
    foreach ($modules as $module => $details) {
        ...
        foreach ($records as $record) {
            ...
            if (itk_progress_default_value($config->{'monitor_'.$module.$record->id}, 0) == 1) {
                $numevents++;
                // Check the time the module is due.
                if (
                    isset($details['defaultTime']) &&
                    $record->due != 0 &&
                    itk_progress_default_value($config->{'locked_'.$module.$record->id}, 0)
                ) {
                    $expected = itk_progress_default_value($record->due);
                } else {
                    $expected = $config->{'date_time_'.$module.$record->id};
                }

                // Gather together module information.
                $coursemodule = itk_block_progress_get_coursemodule($module, $record->id, $course);
                $events[] = array(
                    'expected' => $expected,
                    'type'     => $module,
                    'id'       => $record->id,
    ...
    if (isset($config->orderby) && $config->orderby == 'orderbycourse') {
        usort($events, 'itk_block_progress_compare_events');
    } else {
        usort($events, 'itk_block_progress_compare_times');
    }
    return $events;
}
 ...
 */
function itk_block_progress_compare_events($a, $b) {
 ...
 */
function itk_block_progress_compare_times($a, $b) {
    if ($a['expected'] != $b['expected']) {
        return $a['expected'] - $b['expected'];
    } else {
        return itk_block_progress_compare_events($a, $b);
    }
}
 ...
 */
function itk_block_progress_attempts($modules, $config, $events, $userid, $course) {
    global $DB;
    $attempts = array();
    $modernlogging = false;
    $cachingused = false;
 ...
 */
function itk_block_progress_bar($modules, $config, $events, $userid, $instance, $attempts, $course, $simple = false) {
    global $OUTPUT, $CFG;
    $now = time();
    ...
    $colours = array();
    $colours['attempted_colour'] = "#00b200";
    $colours['notattempted_colour'] = "#A70917";
    $colours['futurenotattempted_colour'] = "#A70917";

    $content = html_writer::start_div('frontpage-course-progress-info-wrapper', array(
        'onmouseleave' => 'M.block_progress.hideInfo('.$instance.','.$userid.');',
    ));
    ...
    if ((!isset($config->orderby) || $config->orderby == 'orderbytime') && $config->displayNow == 1 && !$simple) {
        ...
        $nowstring = get_string('now_indicator', 'theme_bygma');
    ...
    foreach ($events as $event) {
        ...
        $celloptions = array(
            ...
            'id' => '',
            'width' => $width.'%',
            'onmouseenter' => 'M.block_progress.showInfo('.$instance.','.$userid.','.$event['cm']->id.');',
        ...
        if ($attempted === true) {
            ...
            $cellcontent = $OUTPUT->pix_icon(
                               isset($config->progressBarIcons) && $config->progressBarIcons == 1 ?
                               'cross':'blank', '', 'theme_bygma');
        }
        else {
            $celloptions['style'] .= $colours['futurenotattempted_colour'].';';
            $cellcontent = $OUTPUT->pix_icon('blank', '', 'theme_bygma');
        }
    ...
    if (!$simple) {
        if (isset($config->showpercentage) && $config->showpercentage == 1) {
            $progress = itk_block_progress_percentage($events, $attempts);
            $content .= get_string('progress', 'theme_bygma').': ';
    ...
    foreach ($events as $event) {
        ...
        if ($displaydate) {
            ...
            $content .= get_string('time_expected', 'theme_bygma').': ';
 ...
 */
function itk_block_progress_percentage($events, $attempts) {
    $attemptcount = 0;
 ...
 */
function itk_block_progress_course_sections($course) {
    global $DB;
 ...
 */
function itk_block_progress_filter_visibility($events, $userid, $coursecontext, $course = 0) {
    global $CFG, $USER;
    ...
    foreach ($events as $key => $event) {
        ...
        if ($userid == $USER->id) {
            ...
            $coursemodule = itk_block_progress_get_coursemodule($event['type'], $event['id'], $course->id, $userid);
        }
 ...
 */
function itk_block_progress_on_my_page() {
    global $SCRIPT;
 ...
 */
function itk_block_progress_get_course_context($courseid) {
 ...
 * @return stdClass The context object
 */
function itk_block_progress_get_block_context($blockid) {
 ...
 */
function itk_block_progress_get_coursemodule($module, $recordid, $courseid, $userid = 0) {
    global $CFG;
